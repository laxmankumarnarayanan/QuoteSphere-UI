...
const CreditRisk: React.FC = () => {
  const [deals, setDeals] = useState<CreditRiskDeal[]>([]);
  ...
  const fetchSubmittedDeals = async () => {
    try {
      setLoading(true);
      const deals = await creditRiskService.getSubmittedDeals();
      setDeals(deals);
    } catch (err) {
  ...
  const handleAssignDeal = async (dealId: string) => {
    try {
      ...
      const assignment = await creditRiskService.assignDealToCreditRisk(dealId, "Medium");
      
      // Show success message
      alert('Deal assigned successfully!');
      
      // Navigate to assignment details page
      navigate(`/credit-risk-assignment/${assignment.assignmentId}`);
  ...
  if (loading) {
    return (
      <Layout currentPath={[{ label: 'Credit Risk', href: '/credit-risk' }]}>
  ...
  return (
    <Layout currentPath={[{ label: 'Credit Risk', href: '/credit-risk' }]}>
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Credit Risk Dashboard</h1>
          <p className="text-gray-600">Review and assign submitted deals for credit risk assessment</p>
        </div>
