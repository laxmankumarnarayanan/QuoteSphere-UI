...
const Documentation: React.FC = () => {
  const [deals, setDeals] = useState<DocumentationDeal[]>([]);
  ...
  const fetchSubmittedDeals = async () => {
    try {
      setLoading(true);
      const deals = await documentationService.getSubmittedDeals();
      setDeals(deals);
    } catch (err) {
  ...
  const handleAssignDeal = async (dealId: string) => {
    try {
      ...
      const assignment = await documentationService.assignDealToDocumentation(dealId, "Medium");
      
      // Show success message
      alert('Deal assigned successfully!');
      
      // Navigate to assignment details page
      navigate(`/documentation-assignment/${assignment.assignmentId}`);
  ...
  if (loading) {
    return (
      <Layout currentPath={[{ label: 'Documentation', href: '/documentation' }]}>
  ...
  return (
    <Layout currentPath={[{ label: 'Documentation', href: '/documentation' }]}>
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Documentation Dashboard</h1>
          <p className="text-gray-600">Review and assign submitted deals for documentation assessment</p>
        </div>
