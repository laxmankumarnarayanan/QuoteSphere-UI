...
const DocumentationAssignmentDetails: React.FC = () => {
  const { assignmentId } = useParams<{ assignmentId: string }>();
  const navigate = useNavigate();
  const [assignmentDetails, setAssignmentDetails] = useState<DocumentationAssignmentDetails | null>(null);
  ...
  const fetchAssignmentDetails = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://dealdesk-web-app-fqfnfrezdefbb0g5.centralindia-01.azurewebsites.net/api/documentation-deal-assignments/${assignmentId}`);
      if (response.ok) {
      ...
      <div className="p-6">
        ...
        <div className="mb-6">
          ...
          >
            ...
            Back to Documentation Dashboard
          </button>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Documentation Assignment Details</h1>
        ...
        {/* Submit Documentation Analysis Button */}
        <div className="flex justify-end">
          <button
            onClick={() => alert("The Documentation Analysis is submitted. Please update the DealPhase and DealStatus in the Deal table as needed")}
...
export default DocumentationAssignmentDetails; 