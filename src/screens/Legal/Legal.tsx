...
const Legal: React.FC = () => {
  const [deals, setDeals] = useState<LegalDeal[]>([]);
  ...
  const fetchSubmittedDeals = async () => {
    try {
      setLoading(true);
      const deals = await legalService.getSubmittedDeals();
      setDeals(deals);
    } catch (err) {
  ...
  const handleAssignDeal = async (dealId: string) => {
    try {
      ...
      const assignment = await legalService.assignDealToLegal(dealId, "Medium");
      
      // Show success message
      alert('Deal assigned successfully!');
      
      // Navigate to assignment details page
      navigate(`/legal-assignment/${assignment.assignmentId}`);
  ...
  if (loading) {
    return (
      <Layout currentPath={[{ label: 'Legal', href: '/legal' }]}>
    ...
    <Layout currentPath={[{ label: 'Legal', href: '/legal' }]}>
      <div className="p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Review and assign submitted deals for legal assessment</p>
        </div>
