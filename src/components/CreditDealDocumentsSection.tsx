...
import { creditDealDocumentsService, CreditDealDocuments } from '../services/creditDealDocumentsService';
import { BlobServiceClient } from '@azure/storage-blob';

interface CreditDealDocumentsSectionProps {
  dealId: string;
  assignmentId: string;
  readOnly?: boolean;
}

export const CreditDealDocumentsSection: React.FC<CreditDealDocumentsSectionProps> = ({
  dealId,
  assignmentId,
  readOnly = false
}) => {
  const [documents, setDocuments] = useState<CreditDealDocuments[]>([]);
  ...
  const loadDocuments = async () => {
    try {
      setIsLoading(true);
      const data = await creditDealDocumentsService.getDocumentsByAssignmentId(assignmentId);
      setDocuments(data);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        const updated = await creditDealDocumentsService.updateDocument(editingId, documentData);
        setDocuments(prev => prev.map(doc => doc.creditDocumentId === editingId ? updated : doc));
        setEditingId(null);
      } else {
        // Create new document
        const created = await creditDealDocumentsService.createDocument({
          ...documentData,
  ...
  const handleEdit = (document: CreditDealDocuments) => {
    setEditingId(document.creditDocumentId);
    setFormData({
  ...
  const handleDelete = async (documentId: string) => {
    ...
    try {
      await creditDealDocumentsService.deleteDocument(documentId);
      setDocuments(prev => prev.filter(doc => doc.creditDocumentId !== documentId));
    } catch (error) {
  ...
  return (
    <div className="border border-brand-200 rounded-lg bg-brand-50 p-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-brand-900">Credit Risk Documents</h3>
        {!readOnly && (
          <button
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {documents.map((document) => (
            <div key={document.creditDocumentId} className="flex items-center justify-between p-3 border border-brand-200 rounded-lg bg-white">
              ...
              <div className="flex items-center gap-2">
                ...
                {!readOnly && (
                  <>
                    ...
                    <button
                      onClick={() => handleDelete(document.creditDocumentId)}
