...
interface DocumentationDealDocumentsSectionProps {
  dealId: string;
  assignmentId: string;
  readOnly?: boolean;
}

const DocumentationDealDocumentsSection: React.FC<DocumentationDealDocumentsSectionProps> = ({
  dealId,
  assignmentId,
  readOnly = false
}) => {
  ...
  useEffect(() => {
    console.log('DocumentationDealDocumentsSection: assignmentId =', assignmentId);
    fetchDocuments();
  }, [assignmentId]);
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const response = await fetch(`https://dealdesk-web-app-fqfnfrezdefbb0g5.centralindia-01.azurewebsites.net/api/documentation-deal-documents/assignment/${assignmentId}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch('https://dealdesk-web-app-fqfnfrezdefbb0g5.centralindia-01.azurewebsites.net/api/documentation-deal-documents', {
        method: 'POST',
        headers: {
  ...
  const handleEdit = (document: DocumentationDocument) => {
    setEditingId(document.dealDocumentId);
    setFormData({
  ...
  const handleDelete = async (documentId: string) => {
    ...
    try {
      const response = await fetch(`https://dealdesk-web-app-fqfnfrezdefbb0g5.centralindia-01.azurewebsites.net/api/documentation-deal-documents/${documentId}`, {
        method: 'DELETE',
      });

      if (response.ok) {
        setDocuments(documents.filter(doc => doc.dealDocumentId !== documentId));
      } else {
  ...
  return (
    <div className="border border-brand-200 rounded-lg bg-brand-50 p-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-brand-900">Documentation Documents</h3>
        {!readOnly && (
          <button
      ...
      {documents.length === 0 ? (
        ...
        <div className="space-y-3">
          {documents.map((document) => (
            <div key={document.dealDocumentId} className="flex items-center justify-between p-3 border border-brand-200 rounded-lg bg-white">
              ...
              <div className="flex items-center gap-2">
                ...
                {!readOnly && (
                  <>
                    ...
                    <button
                      onClick={() => handleDelete(document.dealDocumentId)}
...
export { DocumentationDealDocumentsSection }; 